...
public partial struct BitRate : IParsable<BitRate>
{
    ...
    public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, [MaybeNullWhen(false)] out BitRate value)
    {
        return Parsable.TryParse<BitRate, BitUnit>(Regex(), s, provider, out value);
    }
    ...
    {
        if (TryParse(s, provider, out BitRate value))
        {
            return value;
        }
